...
/// Verify the permission needed by an incoming remote-control session.
///
/// Accessibility is mandatory for CGEvent mouse/keyboard injection. Permission
/// prompts intentionally belong to the Flutter main-window guide: asking from
/// raw input events can steal focus, race with System Settings, and makes a
/// rejected prompt impossible to explain to the PAD operator.
pub fn ensure_remote_input_permissions() -> bool {
    let accessibility_granted = is_process_trusted(false);

    if !accessibility_granted {
        log::warn!(
            "macOS remote input blocked: grant KEMI-远程桌面 Accessibility permission in the main-window guide to allow mouse and keyboard control"
        );
    }
