...
## 十七、2026-07-29 PAD 触摸与操作栏回退

- 远控触摸手势、底部操作栏和手势帮助页恢复至 `31767b998`（2026-07-28）基线，撤回后续的操作栏压缩、图标文字和双指滚轮改动。
- 保留中文翻译词条；恢复后的帮助页只描述恢复后的实际手势，避免文案与行为不一致。
- 修复自定义多指识别器的状态残留：最后一根手指抬起时立即清空状态，双指操作后的独立单点不会被误判为旧手势的延续。
- `Cargo.toml` 与 `Cargo.lock` 升级为 `1.4.16`；`flutter/pubspec.yaml` 升级为 `1.4.16+74`，Android `versionCode` 为 `74`。
