...
## 0. 当前候选快照：1.4.60+165（2026-08-06）

> 比较基线：GitHub `backup/master = af4cd82db0878e1533af6d1f0ac2c698970288fd`；目标为本文所在最终提交。推送完成后两端应完全同哈希、工作树应为空。

相对该云端基线，本地已包含1.4.59完成的跨屏键盘HOME宿主复用、首页远程ID对屏数字键盘、密码框布局及文件按钮调整；本轮再为PAD自更新加入升级前可选备份当前APK，将源码候选提升到`1.4.60+165`。文件分享、手动安装其他APK、文件传输策略、VP9、鼠标协议和桌面客户端逻辑均未改写。
...
| 构建发布 | 项目隔离Flutter、固定NDK、Android签名门禁、源码产品1.4.60/PAD build165、显示名KEMI远程办公 | 高；全局Flutter会污染`.dart_tool`，分析/pub/build必须使用同一项目工具链。包名和证书未变，桌面三个固定包及`BIN/release`本轮不重建 |

验证：1.4.59跨屏闭环证据继续有效。本轮目标页面定向Flutter analyze无问题；Android arm64 Rust核心按固定NDK增量重编并通过未解析`sodium_*`门禁；固定签名arm64 Release为24,581,112字节，SHA-256 `890a47efa5922af59068f202ee1615cbf5d44340b1384cc54a42e6515e32d065`。包内`1.4.60+165`、`minSdk=22`、包名`com.newlinksz.kemi.remote`，固定证书SHA-256仍为`8546d03e51d09dfa17dbcf432f84bccf74bd2d9fde1cff981ff202f8871871a2`。已覆盖安装到`192.168.3.63:5555`并确认MainActivity在Display 2处于Resumed/Focused，进程存活且无FATAL。遵照效率优先，本轮未跑无关全量回归；升级选择框与Downloads真实落盘留到云端存在更高版本时做两条人工验收。

发布状态：本轮没有修改`BIN/release`。其中PAD仍为已验收的`1.4.59+164`，Mac/Windows/Linux及两份清单也保持原字节；`1.4.60+165`目前仅是源码与本机真机测试候选，不应上传云端。下一次形成正式PAD发布时，需再执行“旧PAD显示更新 → 选择是并检查Downloads备份 → 安装/回读 → 再以选择否验证直接升级”的闭环后，才替换PAD和两份清单。
