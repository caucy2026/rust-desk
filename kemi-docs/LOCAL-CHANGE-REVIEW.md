...
## 0. 当前候选快照：1.4.56+161（2026-08-05）

> 比较基线：本轮开始时GitHub `backup/master = 8fc411d7bcb397e5be13e2b8fd29a8e7c7da69cb`；目标为本文所在最终提交。推送完成后两端应完全同哈希、工作树应为空。

相对该云端基线，本轮新增范围仅为：左侧PAD本地APK安装入口、MainActivity与双屏FileTransferActivity原生安装桥接、FileProvider本地存储路径，以及`1.4.56+161`版本和发布文档。Mac资源监控、PAD双栏传输、连接记录、VP9硬解、键盘稳定、自更新和删除/空间策略已经包含在云端基线中，本轮未再次改写。

| 模块 | 本地相对云端变化 | 风险与当前结论 |
|---|---|---|
| 文件传输 | 左右双栏、持久记录、清除陈旧选择、远端传入文件/目录安全删除、PAD目标50%空间门禁 | 高；只授权成功接收的本地目标。收到目录允许递归删除且包含后来放入的内容，删除后清除整棵授权。8项策略测试通过，仍需远端真实大目录回归 |
| Android更新 | 数值版本比较、自下载/哈希校验/系统安装、PAD行显示本机版本 | 高；云端旧于本机时正确隐藏按钮。上传1.4.56+161后的正向升级尚待远端验收，失败不得删除已安装App |
...
| 构建发布 | 固定Flutter入口、Android签名门禁、产品1.4.56/build161 | 中；APK已全量构建和签名校验。桌面三个固定包本轮没有重建，release必须标注PAD热修订而非四端对齐 |

验证：本轮固定签名arm64 Release构建成功，24,593,361字节，SHA-256 `d383d1f06e783a1e463dba02aa81b42bbd5c7606c4c241ad7d2f476fa76a29a5`；包内`1.4.56+161`，证书未变化，APK内Rust核心与本轮目标文件哈希一致。文件策略8项测试、版本比较4项测试此前通过；本轮定向analyze无error/warning，仅3条既有弃用info，完整Gradle Release构建通过。

发布剩余动作：生成并校验`BIN/release`六文件，推送`backup/master`；管理员本轮上传PAD、SHA清单并最后上传manifest，随后远端验证“旧PAD显示更新 → 下载进度 → 系统安装 → 重启回读1.4.56+161”。
