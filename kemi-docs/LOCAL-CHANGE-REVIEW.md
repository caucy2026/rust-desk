...
## 0. 当前候选快照：1.4.59+164（2026-08-06）

> 比较基线：GitHub `backup/master = af4cd82db0878e1533af6d1f0ac2c698970288fd`；目标为本文所在最终提交。推送完成后两端应完全同哈希、工作树应为空。

相对该云端基线，本轮修正跨屏键盘HOME后的宿主生命周期，增加首页远程ID对屏数字键盘，压缩密码框下方的记住密码行并缩小文件按钮图标，将PAD提升到`1.4.59+164`。文件分享、APK安装、文件传输策略、VP9和鼠标协议不改写。

| 模块 | 本地相对云端变化 | 风险与当前结论 |
|---|---|---|
| 文件传输 | 左右双栏、持久记录、清除陈旧选择、远端传入文件/目录安全删除、PAD目标50%空间门禁 | 高；只授权成功接收的本地目标。收到目录允许递归删除且包含后来放入的内容，删除后清除整棵授权。8项策略测试通过，仍需远端真实大目录回归 |
| Android更新 | 数值版本比较、自下载/哈希校验/系统安装、PAD行显示本机版本 | 高；云端旧于本机时正确隐藏按钮。上传1.4.59+164后的正向升级尚待远端验收，失败不得删除已安装App |
...
| 构建发布 | 项目隔离Flutter、固定NDK、Android签名门禁、产品1.4.59/build164、显示名KEMI远程办公 | 高；全局Flutter会污染`.dart_tool`，分析/pub/build必须使用同一项目工具链。包名和证书未变，桌面三个固定包本轮不重建 |

验证：本轮固定签名arm64 Release构建成功，24,578,154字节，SHA-256 `0a8b787e776565be64159c01ddf0d82cb4692bf9dc1fb35a854a88cdb86e79c6`；包内`1.4.59+164`、`minSdk=22`且显示名“KEMI远程办公”，证书未变化，NDK扫描无未解析`sodium_*`。已覆盖安装到`192.168.3.63:5555`并在Display 2冷启动；首页ID键盘位于Display 0且为数字类型。未授权流程在副屏显示中文用途说明并进入系统权限列表，授权后原点击自动继续。最终包中主屏HOME后0.4秒立即点击可恢复同一键盘task 554；文件页恢复同一task 555。日志明确记录跨屏启动由一次性`SYSTEM_ALERT_WINDOW`权限允许，无FATAL；最终真机`base.apk`哈希与交付APK一致。

发布状态：`BIN/release`替换PAD后重新生成两份清单，Mac/Windows/Linux字节保持不变。管理员剩余动作仅为上传PAD、SHA清单并最后上传manifest，随后远端验证“旧PAD显示更新 → 下载进度 → 系统安装 → 重启回读1.4.59+164”。
