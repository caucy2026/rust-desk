...
## 0. 当前相对稳定快照：1.4.61+166（2026-08-06）

> 比较基线：GitHub `backup/master = af4cd82db0878e1533af6d1f0ac2c698970288fd`；目标为本文所在最终提交。推送完成后两端应完全同哈希、工作树应为空。

相对该云端基线，本地包含跨屏键盘HOME宿主复用、首页数字ID键盘、密码框布局、文件按钮和`1.4.60`自升级前可选备份；当前又完成连接密码对屏输入及三模式宿主交接，并提升为唯一版本`1.4.61+166`。用户已确认同代码的真机行为相对稳定。文件分享、手动安装其他APK、文件传输策略、VP9、鼠标协议和桌面客户端逻辑均未改写。
...
| 构建发布 | 项目隔离Flutter、固定NDK、Android签名门禁、源码产品1.4.61/PAD build166、显示名KEMI远程办公 | 高；全局Flutter会污染`.dart_tool`，分析/pub/build必须使用同一项目工具链。包名和证书未变，桌面三个固定包及`BIN/release`本轮不重建 |

验证：1.4.59跨屏闭环证据继续有效；用户确认包含本地密码对屏输入的真机`1.4.60+165`相对稳定，设备`base.apk`与11:05本地构建均为24,579,999字节、SHA-256 `027a10ee9581a1d45b3d322192a9d96eb709137c27e2c3b1d2718fc60835e535`。为消除同版本不同代码，源码统一提升到`1.4.61+166`并按固定NDK重编Android arm64核心，未解析`sodium_*`门禁通过；最终固定签名Release为24,579,994字节、SHA-256 `29a2ee1c6610fdcec847c8130fbc952bec2f12f8135dd16a8f9ebb2405f02e6d`，包名`com.newlinksz.kemi.remote`、`minSdk=22`、v1/v2签名有效，固定证书SHA-256仍为`8546d03e51d09dfa17dbcf432f84bccf74bd2d9fde1cff981ff202f8871871a2`。遵照效率优先，本轮不重复运行无关全量回归；升级选择框与Downloads真实落盘留到云端存在更高版本时做两条人工验收。

发布状态：本轮仍不修改`BIN/release`。PAD正式上传副本保持`1.4.59+164`，Mac/Windows/Linux及两份清单保持原字节；相对稳定PAD将以带版本号文件归档在`BIN/`根目录。下一次进入正式上传区前，仍需完成自升级备份“是/否”两条远端闭环并重算清单。
