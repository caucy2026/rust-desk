...
## 0. 当前候选快照：1.4.58+163（2026-08-06）

> 比较基线：启动崩溃修复前GitHub `backup/master = 5310a9f9b1d760df69751dc281b032ebd7ba6a51`；目标为本文所在最终提交。推送完成后两端应完全同哈希、工作树应为空。

相对该云端基线，本轮只修复Android arm64 Rust核心的libsodium静态链接构建链、增加未解析符号门禁，并将PAD提升到`1.4.58+163`。`1.4.57`的分享、HOME重开、工具栏和产品名逻辑不再改写。

| 模块 | 本地相对云端变化 | 风险与当前结论 |
|---|---|---|
| 文件传输 | 左右双栏、持久记录、清除陈旧选择、远端传入文件/目录安全删除、PAD目标50%空间门禁 | 高；只授权成功接收的本地目标。收到目录允许递归删除且包含后来放入的内容，删除后清除整棵授权。8项策略测试通过，仍需远端真实大目录回归 |
| Android更新 | 数值版本比较、自下载/哈希校验/系统安装、PAD行显示本机版本 | 高；云端旧于本机时正确隐藏按钮。上传1.4.58+163后的正向升级尚待远端验收，失败不得删除已安装App |
...
| 跨屏键盘/鼠标 | 代理Activity状态机、认证前不抢InputConnection、源窗口保持可聚焦、右键丢release自校准 | 高；已有稳定版本证据，后续不得重新加入`FLAG_NOT_FOCUSABLE` |
| 分享与安装 | 左侧普通文件可分享、APK还可安装；右侧文件和文件夹不增加操作 | 中；系统Chooser/Installer拥有最终确认权，FileProvider只授予单URI读取；正式构建通过，待真机验证分享目标过滤 |
| HOME重开 | 停止的键盘和文件辅助任务同步清状态并结束，不等待后台回调 | 高；解决busy/隐藏singleInstance根因，但必须真机复测键盘、文件页、右键和远控视频连续性 |
| 构建发布 | 固定Flutter入口、Android签名门禁、产品1.4.58/build163、显示名KEMI远程办公 | 高；除签名/哈希外新增真实ELF未解析sodium符号门禁和副屏冷启动验证。包名和证书未变，桌面三个固定包本轮不重建 |

验证：本轮固定签名arm64 Release构建成功，24,664,810字节，SHA-256 `1180d3d4ff2c1f9387665dcabcc502cf08d1ec1b3f3f9ed4be452629f433c536`；包内`1.4.58+163`且显示名“KEMI远程办公”，证书未变化。APK内Rust核心SHA-256为`ac30f9d82f548a1d514c29652487db0bbc50539320b22058aa9a85df88fafa77`，NDK扫描无未解析`sodium_*`。已覆盖安装到`192.168.3.63:5555`并在Display 2冷启动，进程持续存活且日志无FATAL。

发布状态：`BIN/release`替换PAD后重新生成两份清单，Mac/Windows/Linux字节保持不变。管理员剩余动作仅为上传PAD、SHA清单并最后上传manifest，随后远端验证“旧PAD显示更新 → 下载进度 → 系统安装 → 重启回读1.4.58+163”。
