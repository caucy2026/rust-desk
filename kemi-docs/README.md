...
**KEMI-远程办公** 是基于 [RustDesk](https://github.com/rustdesk/rustdesk) (AGPL-3.0) 定制的远程桌面客户端，面向 Android PAD 与多平台远控场景。当前本地待上传批次为 **1.4.78+185 PAD第一阶段封版**：PAD使用固定Newlink release签名，macOS仍为已完成Developer ID签名、公证和票据装订的`1.4.75+182`，Windows/Linux保持`1.4.75`原字节；Newlink线上实际版本仍以固定HTTPS接口回读的`release-manifest.json`为准。`BIN/`根目录保存带版本号的不可变归档，`BIN/release/`保存无版本号、永久固定名称的云盘上传副本。
...
- 当前PAD源码`1.4.78+185`完成第一阶段界面封版：顶栏统一为“KEMI远程办公 v1.4.78”，服务器正常状态统一为绿色圆点加“就绪”；`1.4.77`的单屏工具同屏适配和既有双屏行为全部保留。
- `BIN/release`形成PAD `1.4.78+185`加三端既有`1.4.75`的混合发布快照；管理员尚未上传Newlink云盘，因此manifest状态为`pending-manual-upload`。本轮只需上传PAD、SHA256SUMS，最后上传release-manifest，随后再做云端回读和PAD远程升级闭环。
