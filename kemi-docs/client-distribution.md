...
## 当前云端正式批次：1.4.49+154；待上传PAD热修订：1.4.55+160

上传前Newlink云端仍是四端`1.4.49+154`完整批次。应远端自更新验证要求，本次`BIN/release`形成明确标注的PAD热修订快照：Android替换为`1.4.55+160`，Mac/Windows/Linux保持已验收的1.4.49原字节，两份清单按真实字节重算。它不是“四端均为1.4.55”，任何文档或UI都不得如此宣称。
...
管理员按固定顺序上传六文件并最后上传manifest后，低于`1.4.55+160`的PAD才应显示按钮；下载、系统安装、重启后版本回读是本次远端验收项。
...
源码产品版本来自根`Cargo.toml`，当前为`1.4.55`；本次仅PAD重建为`1.4.55+160`。macOS、Windows、Linux固定文件仍来自正式1.4.49批次，版本和SHA必须如实保留。下一次完整四端发布必须从同一提交重建全部客户端后，才能取消“PAD热修订/混合批次”标记。
