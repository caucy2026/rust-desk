...
- 当前版本源必须一致：Cargo.toml=1.4.10、Cargo.lock rustdesk=1.4.10、flutter/pubspec.yaml=1.4.10+68。
...
| 文件 | 当前值 |
|---|---|
| `Cargo.toml` | `version = "1.4.10"` |
| `Cargo.lock` 的 rustdesk package | `version = "1.4.10"` |
| `flutter/pubspec.yaml` | `version: 1.4.10+68` |
...
必须读取 APK 的 PackageInfo。不要改回 `bind.mainGetVersion()`，因为当前预编译 Rust `.so` 可能返回旧版本 `1.4.9`，会出现 APK 是 1.4.10、首页却显示 1.4.9 的假象。
