...
当前版本为 1.4.13+71，文件传输并行浮窗的功能代码基线为 8f4c18c57。文档提交和后续开发会使 HEAD 继续前进，必须用 git rev-parse HEAD 和 git ls-remote backup refs/heads/master 核对当前本地与远端提交。
...
- 当前版本源必须一致：Cargo.toml=1.4.13、Cargo.lock rustdesk=1.4.13、flutter/pubspec.yaml=1.4.13+71。
...
| 文件 | 当前值 |
|---|---|
| `Cargo.toml` | `version = "1.4.13"` |
| `Cargo.lock` 的 rustdesk package | `version = "1.4.13"` |
| `flutter/pubspec.yaml` | `version: 1.4.13+71` |
...
必须读取 APK 的 PackageInfo。不要改回 `bind.mainGetVersion()`，因为当前预编译 Rust `.so` 可能返回旧版本 `1.4.9`，会出现 APK 是 1.4.13、首页却显示 1.4.9 的假象。
